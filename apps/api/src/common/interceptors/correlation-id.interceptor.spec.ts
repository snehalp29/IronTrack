...
describe('CorrelationIdInterceptor', () => {
  ...
  it('falls back to a generated id when the provided header contains invalid characters', async () => {
    const { context, request, setHeader } =
      createExecutionContext('bad\nheader');
  ...
  it('uses the first valid correlation id when the header array starts with invalid values', async () => {
    const { context, request, setHeader } = createExecutionContext([
      '   ',
      'bad\nheader',
      'valid-id',
    ...
    expect(request.headers['x-correlation-id']).toBe('valid-id');
    expect(setHeader).toHaveBeenCalledWith('x-correlation-id', 'valid-id');
  });
});
